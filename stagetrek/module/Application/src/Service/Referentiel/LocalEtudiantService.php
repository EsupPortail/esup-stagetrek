...
use Application\Service\Referentiel\Entity\LocalEtudiantResult;
...
class LocalEtudiantService implements RechercheEtudiantServiceInterface, ObjectManagerAwareInterface
{
...
//        return $res;
     ...
     */
    public function findEtudiantByMail(string $email, int $limit=-1) : ?ReferentielEtudiantInterface
    {
        $etudiant = $this->getObjectManager()->getRepository(Etudiant::class)->findOneBy(['mail' => $mail]);
        if(!isset($etudiant)){return null;}
        return new LocalEtudiantResult($etudiant);
    }
