...
class FilesystemStorageAdapter extends AbstractStorageAdapter
{
    ...
    {
        $toDirPath = rtrim(trim($toDirPath), "/");
        $this->createDirectory($toDirPath);

//        Si le fileName contient des sous-repertoires non définis, on les créés
        $subDir = explode($this->pathSeparator, $toFileName);
        array_pop($subDir);
        $subDir = implode($this->pathSeparator, $subDir);
        $subPath = $this->assemblePath($toDirPath, $subDir);
