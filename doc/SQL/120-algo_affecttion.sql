-- Date de MAJ 09/10/2024 ----------------------------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS procedure_affectation (
...
CREATE UNIQUE INDEX IF NOT EXISTS procedure_affectation_code_unique ON "procedure_affectation" ( code );

INSERT INTO procedure_affectation (code, libelle, description, ordre)
VALUES ('v1', 'Version 1', '<p><strong>Algorithme d''affectation V2</strong><br /><em>(V1 simplifiée)</em></p><ul><li>Affecte en respectant au plus proche les préférences des étudiants, les places disponible et l''ordre pré-définie</li><li>Coût des terrains en fonction des places disponibles, du nombre de demandes, du rang de la préférence satisfaite</li><li>Bonus / Malus uniquement en fonction de l''ordre d''affectation</li></ul>', 1)
	 , ('v2', 'Version 2', '<p><strong>Algorithme d''affectation V2</strong><br /><em>(V1 simplifiée)</em></p><ul><li>Affecte en respectant au plus proche les préférences des étudiants, les places disponible et l''ordre pré-définie</li><li>Coût des terrains en fonction des places disponibles, du nombre de demandes, du rang de la préférence satisfaite</li><li>Bonus / Malus uniquement en fonction de l''ordre d''affectation</li></ul>', 2)
ON CONFLICT (code)
...
WITH data( code, lib, ordre ) AS (
	SELECT  'procedure_afficher', 'Appliquer les procédures d''affectation de stage ', 51
	UNION
	SELECT  'procedure_modifier', 'Modifier la description des procédures d''affectation de stage ', 52
)
INSERT
...
WITH data ( code_privilege, code_role ) AS (
	SELECT 'procedure_afficher', 'Admin_tech'
	UNION
	SELECT 'procedure_afficher', 'Admin_fonc'
	UNION
	SELECT 'procedure_modifier', 'Admin_tech'
	UNION
	SELECT 'procedure_modifier', 'Admin_fonc'
)
   , role_linker AS (
...
----
-- NOTE
-- Choix fait actuellement : passage par une procédure SQL
-- Avantage : temps de calcul car lourdeurs d'excution en PHP
-- Inconvenient : Difficile a débugguer
----
...
CREATE
	or replace PROCEDURE algo_score_v1(sessionId integer)
	LANGUAGE plpgsql
AS
$$
declare
...
--  **/
	   ...
	   , bonus_malus as (
	                     select a.affectation_id, a.terrain_id, case
		                     when terrain_id = 64 then 0 -- cas de la MG : pas de bonus/malus todo : en faire un paramètre
...
CREATE
	or replace PROCEDURE algo_score_v2(sessionId integer)
	LANGUAGE plpgsql
AS
$$
declare
...
-- cout fixe ajouter après
	...
	with affectation as (select tmp.ordre_affectation   as ordre_affectation,
	   ...
	   , data_bonus as (select
		                    max(a.ordre_affectation) as nb_etudiants,
		                    round(max(a.ordre_affectation)/2) as rang_moyen,
		                    0.1 as base_malus
	                    from affectation a
	                                                         )
	   , bonus_malus as (
	                     select a.affectation_id, a.terrain_id,
		                     (data.rang_moyen - a.ordre_affectation) * data.base_malus
			                     as bonus_malus
	                     from affectation a, data_bonus data
	                                                         )
	   , bonus_malus_final as (
	                     select a.id as affectation_id, case
		                     when a.validee or a.pre_validee then a.cout_terrain
		                     when tmp.rang_preference = 0 or tmp.terrain_stage_id is null then 0
		                                                    else least(round(cast(tmp.cout_terrain + bm.bonus_malus as numeric), cast(precision.value as int)), cast(borne_max.value as integer))
	                                                    end as cout, case
		                     when a.validee or a.pre_validee then a.bonus_malus
		                     when tmp.rang_preference = 0 or tmp.terrain_stage_id is null then 0
		                                                                 else bm.bonus_malus
	                     ...
	                     from tmp_affectation tmp
		                          join affectation_stage a
		                          join parametre borne_max on borne_max.code = 'cout_total_max'
		                          join parametre precision on precision.code = 'precision_cout_affectation'
		                          on tmp.affectation_id = a.id
		                          left join bonus_malus bm on a.id = bm.affectation_id
	                                                         )
	update tmp_affectation tmp
